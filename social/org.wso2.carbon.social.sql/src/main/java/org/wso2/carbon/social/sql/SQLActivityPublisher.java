...
public class SQLActivityPublisher extends ActivityPublisher {
	...
	private static String insertCommentSQL = null;
	private static String insertRatingSQL= null;
	private static String insertLikeSQL= null;
	...
	private long publishPostActivity(SQLActivity activity, JsonObject jsonObject)
		...
		try {
			...
			if (insertCommentSQL == null) {
				//TODO remove info log
				log.info("insertCommentSQL not found. setting up.. ");
				insertCommentSQL= SocialUtil.getSelectSQL(connection, "SOCIAL_COMMENTS", "insert");
			}
			
			if (log.isDebugEnabled()) {
				log.debug("Executing: " + insertCommentSQL);
			}
			//Oracle JDBC driver does not support Statement.RETURN_GENERATED_KEYS hence using following
			String generatedColumns[] = { "id" };
			commentStatement = connection.prepareStatement(insertCommentSQL,
					generatedColumns);
			//commentStatement = connection.prepareStatement(insertCommentSQL);
			...
			//commentStatement.setString(8, uuid);
			...
			if (!generatedKeys.next()) {
				...
				if (rating > 0) {
					...
					if (insertRatingSQL == null) {
						//TODO remove info log
						log.info("insertRatingSQL not found. setting up.. ");
						insertRatingSQL= SocialUtil.getSelectSQL(connection, "SOCIAL_RATING", "insert");
					}
					
					if (log.isDebugEnabled()) {
						log.debug("Executing: " + insertRatingSQL);
					}

					ratingStatement = connection
							.prepareStatement(insertRatingSQL);
	...
	private void insertLikeActivity(SQLActivity activity, int likeValue,
			Connection connection) throws SQLException, SocialActivityException {
		...
		try {
			if (insertLikeSQL == null) {
				// TODO remove info log
				log.info("insertLikeSQL not found. setting up.. ");
				insertLikeSQL = SocialUtil.getSelectSQL(connection,
						"SOCIAL_LIKES", "insert");
			}

			if (log.isDebugEnabled()) {
				log.debug("Executing: " + insertLikeSQL);
			}

			insertActivityStatement = connection
					.prepareStatement(insertLikeSQL);
